  ...
  try {
    const execution = await new Promise((resolve, reject) => {
      client.getQueryExecution({QueryExecutionId: id}, (err, data) => {
        ...
        // } else { 
        //   consile.log('I\'m waiting...')
        // }
      });
    });
    if (event.Records[0].messageAttributes.IsPending) {
      execution.Status.State = 'PENDING';
      delete execution.ResultConfiguration.OutputLocation;
      delete execution.Status.CompletionDateTime;
      delete execution.Statistics;
      var params = {
